...
cdef class DLX:
    ...
    cdef list columns
    cdef list nodes
    cdef list solution  # Keep for API compatibility
    cdef list solutions
    cdef int num_rows
    cdef int num_cols
    cdef bool find_all
    cdef bint _solution_found  # Flag for early termination
    # C array for solution stack (much faster than Python list)
    cdef int[1000] solution_stack
    cdef int solution_depth
    ...
    def __init__(self, int num_rows, int num_cols, object matrix):
        ...
        self.columns = []
        self.nodes = []
        self.solution = []  # Keep for API compatibility
        self.solutions = []
    ...
    cdef void cover(self, Column col):
        ...
        cdef Node j
        cdef Column jcol  # Cache column pointer
        while i != col:
            j = i.right
            while j != i:
                jcol = j.column  # Cache column pointer
                j.down.up = j.up
                j.up.down = j.down
                jcol.size -= 1
                j = j.right
    ...
    cdef void search(self, int k):
        ...
        if self.header.right == self.header:
            # Found a solution - copy from C array to Python list (optimized)
            sol_list = [self.solution_stack[i] for i in range(self.solution_depth)]
            self.solutions.append(sol_list)
        ...
        while r != c:
            row_id = r.row_id
            # Use C array stack instead of Python list
            self.solution_stack[self.solution_depth] = row_id
            ...
            if not self.find_all and self._solution_found:
                ...
                while j != r:
                    ...
                    self.uncover(jcol)
                    j = j.left
                self.solution_depth -= 1  # Pop from C array stack
                self.uncover(c)
                return
